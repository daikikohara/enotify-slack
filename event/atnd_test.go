...
func TestGetAtnd(t *testing.T) {
	...
	atnd := new(Atnd)
	events, err := atnd.Get(tsSuccess.URL+"?", keyword, nickname, place)
	if err != nil {
		t.Error(err.Error())
	}
	...
	// invalid url
	events, err = atnd.Get("", keyword, nickname, place)
	if err == nil {
		t.Error(err.Error())
	}
	...
	events, err = atnd.Get(tsInvalidTime.URL+"?", keyword, nickname, place)
	if err != nil {
		t.FailNow()
	}
