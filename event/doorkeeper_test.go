...
func TestGetDoorkeeper(t *testing.T) {
	...
	dk := new(Doorkeeper)
	events, err := dk.Get(tsSuccess.URL+"?", "summary", nickname, place)
	if err != nil {
		t.Error(err.Error())
	}
	...
	// invalid url
	events, err = dk.Get("", keyword, nickname, place)
	if err == nil {
		t.Error(err.Error())
	}
	...
	events, err = dk.Get(tsInvalidTime.URL+"?", "summary", nickname, place)
	if err != nil {
		t.FailNow()
	}
