...
func TestGetConnpass(t *testing.T) {
	...
	events, err := connpass.Get(tsSuccess.URL+"?", keyword, nickname, place)
	if err != nil {
		t.Error(err.Error())
	}
	if !reflect.DeepEqual(eventok, events[0]) {
		t.Error("connpass return value is unexpected. expected and actual are")
		t.Error(eventok)
		t.Error(events[0])
		t.FailNow()
	}
	...
	// invalid url
	events, err = connpass.Get("", keyword, nickname, place)
	if err == nil {
		t.Fail()
	}
	...
	events, err = connpass.Get(tsInvalidTime.URL+"?", keyword, nickname, place)
	if err != nil {
		t.Fail()
	}
